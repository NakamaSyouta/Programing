...
{
	//Bボタンが押されたら、タイトルに戻る
	if (InputControl::GetButtonDown(XINPUT_BUTTON_DPAD_B))
	{
...
void RankingDispScene::Draw()const
{
	...
	for (int i = 0;i < 5;i++)
	{
		DrawFormatString(50, 170 + i * 25, 0xffffff, "%2d %-15s %6d", ranking->GetRanking(i), ranking->GetScore(i));

	}
}
//終了時処理
void RankingDisoScene::Finalize()
{
	//読み込んだ画像の削除
