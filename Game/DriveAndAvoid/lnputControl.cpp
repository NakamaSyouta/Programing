...
void InputControl::Update()
{
	...
	//トリガー入力値の更新の更新(0.0f~1.0fに範囲を制限する)
	trigger[0] = (float)input_state.LeftTrgger / (float)UCHAR_MAX;
	trigger[1]=(float)input_state.RightTrgger / (float)UCHAR_MAX;
	...
	{
		stick[1].x = -((float)input_state.ThubRX / (float)SHRT_MIN);
	}
	if (input_state.ThumbRY > 0.0f)
	{
		stick[1].y = (float)input_state.ThubRY / (float)SHRT_MAX;
	}
	else
	{
...
//左スティック取得
Vector2D InputControl::GetLeftTStick()
{
	return stick[0];
}
//右スティック取得
